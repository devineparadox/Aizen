...
from Aizen.misc import SUDOERS
import asyncio
from pyrogram import Client, filters, enums
from pyrogram.types import Message
from Aizen import app, Userbot
from Aizen.utils.database import get_assistant
...
async def banall(client: Client, message: Message):
    ...
    TgBotOK = await message.reply_text(f"Hey {message.from_user.mention}, 'deleteall' checking...")
...
async def handle_callback(client: Client, callback_query: CallbackQuery):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
...
async def banall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'banall' can only be executed by the group owner {owner_TgBot}.")
        return
    confirm_msg = await message.reply(
...
async def unbanall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unbanall' can only be executed by the group owner {owner_TgBot}.")
        return
    confirm_msg = await message.reply(
...
async def unmuteall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unmuteall' can only be executed by the group owner {owner_TgBot}.")
        return
...
async def muteall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'muteall' can only be executed by the group owner {owner_TgBot}.")
        return
    confirm_msg = await message.reply(
...
async def kickall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'kickall' can only be executed by the group owner {owner_TgBot}.")
        return
    confirm_msg = await message.reply(
...
async def unpinall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_TgBot = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unpinall' can only be executed by the group owner {owner_TgBot}.")
        return
    confirm_msg = await message.reply(
