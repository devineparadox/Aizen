...
from Aizen import app
from Aizen.utils.database import is_on_off
    ...
    if await is_on_off(2):
        ...
        async for admin in app.get_chat_members(message.chat.id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
            if admin.status == enums.ChatMemberStatus.OWNER:
                owner_TgBot = admin.user.mention if hasattr(admin.user, 'mention') and admin.user.mention else "Is_Hide / Deleted"
                owner_TgBot_id = admin.user.id if hasattr(admin.user, 'id') else "Is_Hide / Deleted"
        logger_text = f"""
...
<b>◈ 𝐂𝗵𝗮𝘁 𝗢𝘄𝗻𝗲𝗿 ➪ </b> {owner_TgBot} 𝐈𝐝 ➪<code>{owner_TgBot_id}</code>
<b>◈ 𝐒𝐞𝐚𝐫𝐜𝐡𝐞𝐝 ➪ </b> <code>{message.text.split(None, 1)[1]}</code>
<b>◈ 𝐁𝐲 ➪ </b> {streamtype}
╚═══❰ #𝐍𝐞𝐰𝐒𝐨𝐧𝐠 ❱══❍⊱❁۪۪"""
        if message.chat.id != LOG_ID:
            try:
